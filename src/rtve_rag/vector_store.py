...
def upsert_chunks(client: QdrantClient, collection: str, chunks: list[dict], vectors: list[list[float]]) -> None:
    ...
    points = [PointStruct(id=point_id(chunk["chunk_id"]), vector=vector, payload=chunk) for chunk, vector in zip(chunks, vectors)]
    client.upsert(collection_name=collection, points=points, wait=True)


def delete_chunks(client: QdrantClient, collection: str, chunk_ids: list[str]) -> None:
    if chunk_ids:
        client.delete(collection_name=collection, points_selector=PointIdsList(points=[point_id(chunk_id) for chunk_id in chunk_ids]), wait=True)
