...
def next_id(output: Path, prefix: str) -> int:
    values=[]
    if output.exists():
...
def indexes(value, size):
    ...
    return selected
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--questions', required=True); parser.add_argument('--output', default='eval/questions.jsonl'); parser.add_argument("--id-prefix", default="eval"); parser.add_argument('--endpoint', default='http://127.0.0.1:8000/ask'); parser.add_argument('--top-k', type=int, default=4); parser.add_argument('--program'); parser.add_argument('--emission-date'); args=parser.parse_args()
    output=Path(args.output); output.parent.mkdir(parents=True, exist_ok=True); number=next_id(output, args.id_prefix)
    ...
    for question in questions:
        ...
        case={'id':f'{args.id_prefix}-{number:03d}','question':question,'program':args.program,'emission_date':args.emission_date,'expected_evidence':bool(selected),'expected_chunk_ids':[sources[item-1]['chunk_id'] for item in selected],'notes':notes}
