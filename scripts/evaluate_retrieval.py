...
def main() -> None:
    ...
    parser.add_argument('--min-recall', type=unit_interval); parser.add_argument('--min-mrr', type=unit_interval); parser.add_argument('--min-negative-accuracy', type=unit_interval); parser.add_argument('--normalize-query', action='store_true'); parser.add_argument('--min-score', type=float)
    ...
    for case in cases:
        ...
        results = search(case['question'], client, settings.qdrant_collection, settings.ollama_base_url, settings.ollama_embedding_model, args.top_k, case.get('program'), case.get('emission_date'), normalize=args.normalize_query, min_score=args.min_score)
