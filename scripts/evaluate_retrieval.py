...
def unit_interval(value: str) -> float:
    number = float(value)
    if not 0 <= number <= 1:
        raise argparse.ArgumentTypeError('El umbral debe estar entre 0 y 1.')
    return number
...
def main() -> None:
    ...
    parser.add_argument('--min-recall', type=unit_interval)
    parser.add_argument('--min-mrr', type=unit_interval)
    ...
    report = {'generated_at': datetime.now(timezone.utc).isoformat(), 'input': args.input, 'top_k': args.top_k, 'embedding_model': settings.ollama_embedding_model, 'collection': settings.qdrant_collection, **summarize(scored)}
    failures = threshold_failures(report, args.min_recall, args.min_mrr)
    report['thresholds'] = {'min_recall': args.min_recall, 'min_mrr': args.min_mrr, 'passed': not failures, 'failures': failures}
    ...
    if failures:
        print(json.dumps({'threshold_failures': failures}, ensure_ascii=False), file=sys.stderr)
        raise SystemExit(1)
